...
# Common circuit definition (without SL sources and sim commands)
run_case() {
    local A=$1 B=$2 SLA=$3 SLB=$4 LOGFILE=$5

    $NGSPICE -b -o "$LOGFILE" << NGEOF
.title XOR Debug A=${A} B=${B}
...
.control
run
* BL voltages just before SAE rises (at 9.9ns, during precharge)
meas tran vbl0_pre FIND v(bl0) AT=9.9n
meas tran vbl1_pre FIND v(bl1) AT=9.9n
meas tran vbl0_bl1_diff PARAM='vbl0_pre - vbl1_pre'

* BL voltages just after SAE rises (at 10.5ns)
meas tran vbl0_eval FIND v(bl0) AT=10.5n
meas tran vbl1_eval FIND v(bl1) AT=10.5n

* SA internal nodes at 10.5ns
meas tran vfn1 FIND v(xsa1.fn1) AT=10.5n
meas tran vfn2 FIND v(xsa1.fn2) AT=10.5n
meas tran vtail FIND v(xsa1.tail) AT=10.5n

* SA outputs at 28ns (well into evaluate)
meas tran vq1_eval FIND v(q1) AT=28n
meas tran vqb1_eval FIND v(qb1) AT=28n

* Check equalize transistor MP5 effect: Q-QB at various times
meas tran vq1_11n FIND v(q1) AT=11n
meas tran vqb1_11n FIND v(qb1) AT=11n
meas tran vq1_12n FIND v(q1) AT=12n
meas tran vqb1_12n FIND v(qb1) AT=12n
...
echo "============================================="
echo "  XOR Debug: BL voltages & SA internal nodes"
echo "============================================="
...
for A in 0 1; do
  for B in 0 1; do
    ...
    LOGFILE="/tmp/xor_debug_${A}_${B}.log"

    run_case "$A" "$B" "$SLA" "$SLB" "$LOGFILE" 2>/dev/null

    echo ""
    echo "--- A=$A, B=$B (XOR=$(( A ^ B ))) ---"
    echo "BL precharge (9.9ns):"
    grep -E "vbl0_pre|vbl1_pre|vbl0_bl1_diff" "$LOGFILE" | grep "="
    echo "BL evaluate (10.5ns):"
    grep -E "vbl0_eval|vbl1_eval" "$LOGFILE" | grep "="
    echo "SA internals (10.5ns):"
    grep -E "vfn1|vfn2|vtail" "$LOGFILE" | grep "="
    echo "SA outputs (28ns):"
    grep -E "vq1_eval|vqb1_eval" "$LOGFILE" | grep "="
    echo "SA outputs timeline:"
    grep -E "vq1_11n|vqb1_11n|vq1_12n|vqb1_12n" "$LOGFILE" | grep "="
  done
done
